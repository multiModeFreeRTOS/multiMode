...
#define sysTIMER_BASE                    		( (unsigned long ) 0x20003000 )
...
static volatile test * const regs = (test *)(sysTIMER_BASE);
/*-----------------------------------------------------------*/
...
{
#if configANALYSE_OVERHEAD
	unsigned long t1 = xGetClkCount();
#endif
	vTaskIncrementTick();
...
// unsigned int irqBasic = GET32(BCM2835_BASE_INTC);
// unsigned int pending1 = GET32(BCM2835_IRQ_PENDING1);
// unsigned int pending2 = GET32(BCM2835_IRQ_PENDING2);
// char bof[128];
// snprintf_473(bof, sizeof(bof), "\nBasic:0x%08x\nP1:0x%08x\nP2:0x%08x", irqBasic,pending1,pending2);
// const char * strs = bof;
// uart_puts(strs);

#if configANALYSE_OVERHEAD
	unsigned long t2 = xGetClkCount();
	if (t2 - t1 > 20) {
		char buf[10];
